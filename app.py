...
def rezervacija(id, mode):
    if request.method == 'GET':
        ...
        return render_template('rezervacija.html', knjiga = knjiga, tip = ulogovan())
    else:
        if int(mode):
...
def knjiga_izmena(id):
    if request.method == 'GET':
        ...
        return render_template('knjiga_izmena.html', knjiga = knjiga, tip = 3)
    else:
        forma = request.form
